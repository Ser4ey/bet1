from chromdriver_class import FireFoxDriverWithProxy, ChromeCloudFlareProtection
import time
import data
import datetime
import telegram_api
...
for i in range(len(data.Accounts)):
    ...
    driver2 = FireFoxDriverWithProxy(path_to_geckodriver=data.path_to_geckodriver, user_agent=data.Accounts[i][0],
                                 proxy=data.Accounts[i][1], proxy_login_and_password=data.Accounts[i][2], type_of_account=data.Accounts[i][6], final_balance=data.Accounts[i][7], account_code_name=data.Accounts[i][8])
